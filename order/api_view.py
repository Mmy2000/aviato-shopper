...
def stripe_webhook(request):
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        try:
            # Assume you pass `order_id` in Stripe's metadata when creating the session
            order_id = session['metadata']['order_id']
            user_id = session['metadata']['user_id']

            # Get the order
            order = Order.objects.get(id=order_id, user_id=user_id, is_orderd=False)
            ...
            # Process cart items and store them in OrderProduct
            ...
            for item in cart_items:
                ...
                if product.stock >= item.quantity:
                    ...
                    logger.warning("Insufficient stock for product %s", product)

            # Clear the cart
            cart_items.delete()

        except Order.DoesNotExist:
            logger.error("Order does not exist for Stripe session: %s", session['id'])
...
class CreateStripePaymentView(APIView):
    ...
    def post(self, request):
        ...
        if not success_url or not cancel_url:
            return Response({"error": "Missing success_url or cancel_url"}, status=status.HTTP_400_BAD_REQUEST)
                ...
                line_items=[
                    {
                        "price_data": {
                            ...
                            "unit_amount": int(grand_total * 100),  # Amount in cents
                        },
                        "quantity": 1,
                    }
                ],
